...
# Set help text
help="
barrnapper_5s finds and concatenates all 5S rRNA sequences in the genomes in the genome folder

Syntax:
    barrnapper_5s.sh [ genome_folder ] [ options ]
...
barrnap_func() {
    ...
    rna_5S_file=$( echo $fa_file | sed "s/.fa/.5S/i")
    rna_5S_titles=$( echo $fa_file | sed "s/.fa/.title/i")
    if [[ -s $rna_file ]] ; then
        rm $rna_file
    fi
    if [[ -s $fa_file ]] ; then
    ...
    if [[ ! -s $rna_5S_file ]] ; then
        echo WARNING! empty 5S rRNA file $rna_5S_file
        echo '' > $rna_5S_titles
    fi
    if [[ ! -s $rna_file ]] ; then
...
logt - Concatenating 5S rRNA sequences into one file...
